...
public class MusicStoreLauncher {
    ...
    private static void createViews() throws SQLException{
        ...
        stmt.executeUpdate("CREATE VIEW V_STOCK_INFO "
                + "(STOCK_ID, ALBUM_ID, ALBUM_NAME, ARTIST, VENDOR_NAME, FORMAT, PRICE, IN_STOCK, ON_ORDER, VENDOR_PRICE) "
                + "AS "
                + "SELECT S.STOCK_ID, S.ALBUM_ID, AL.ALBUM_NAME, AR.ARTIST_NAME, V.VENDOR_NAME, S.FORMAT, S.SALE_PRICE, S.AMOUNT, S.ON_ORDER, S.VENDOR_COST "
        ...
        stmt.executeUpdate("CREATE VIEW V_CUSTOMER_INFO "
                + "(NAME,ORDERS,TOTAL_REVENUE,PHONE,ADDRESS,CITY,STATE,ZIP ) "
                + "AS "
                + "SELECT C.NAME, COUNT(DISTINCT(S.SALE_ID)), SUM(SOL.COST), C.CONTACT_NUMBER, C.ADDRESS, C.CITY, C.STATE, C.ZIP "
                + "FROM CUSTOMER C, SALE S, SALE_ORDER_LINE SOL "
                + "WHERE C.CUSTOMER_ID = S.CUSTOMER_ID AND S.SALE_ID = SOL.SALE_ID "
                + "GROUP BY C.NAME, C.CONTACT_NUMBER, C.ADDRESS, C.CITY, C.STATE, C.ZIP");
        conn.commit();
        stmt.close();
    }
